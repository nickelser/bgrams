...
$.widget("ui.multidraggable", $.extend({}, $.ui.draggable.prototype, {
  ...
  destroy: function() {
    if(!this.element.data('multidraggable')) return null;
    this.element
  ...
  _mouseStart: function(event) {
    if (!this.starting_event) {
      return this._mouseStartHelper(event);
    } else {
      this.starting_event = event; // rawr
    }
    return null;
  },
  
  _mouseStartHelper: function(event, direction) {
    var o = this.options;
    
    if (this.element.hasClass('ui-multidraggable')) {
      // set all multidraggables
      if (direction) {
        this._trigger("set_tile_group", event, direction);
      }
  ...
  mouseStart: function(event, draggedOptions) {
    var o = this.options, is_multi = this.element.hasClass('ui-multidraggable');
    ...
    if (this._trigger("start", event, is_multi) === false) {
      this._clear();
      return false;
    }
  ...
  mouseDrag: function(event, noPropagation, options) {
  ...
  _mouseDrag: function(event, noPropagation) {
    if (this.starting_event) {
      var dx = event.pageX - this.starting_event.pageX,
          dy = this.starting_event.pageY - event.pageY,
          dx_b = Math.abs(dx) > this.options.directionThreshold,
          dy_b = Math.abs(dy) > this.options.directionThreshold,
          dx_b2 = Math.abs(dx) > this.options.directionThreshold*0.66,
          dy_b2 = Math.abs(dy) > this.options.directionThreshold*0.66;
      ...
      if (dx_b || dy_b) {
        ...
        if (dx_b && !dy_b2) {
          ...
          if (dy > 0 && dx > 0) { direction = 'ne'; }
          else if (dy < 0 && dx > 0) { direction = 'se'; }
          else if (dy < 0 && dx < 0) { direction = 'sw'; }
          else if (dy > 0 && dx < 0) { direction = 'nw'; }
        }
        
        this._mouseStartHelper(this.starting_event, direction);
        delete this.starting_event;
      } else {
        // still not enough to trigger
        return false;
      }
    }
